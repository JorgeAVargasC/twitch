    ...
    <>
      ...
      {modalGame && (
        <div className='animate__animated animate__fadeIn fixed z-50 top-0 w-full min-h-screen flex justify-center items-center'>
          ...
          <div className='animate__animated animate__fadeInDown z-50 w-3/4 md:w-1/4 bg-gray-900 rounded-lg'>
            ...
            <img
              src={modalGame.box_art_url.replace('{width}', '300').replace('{height}', '400')}
      ...
      <div className='w-11/12 mt-20 mb-20 flex flex-col justify-center items-center'>
        ...
        <input
          ...
          className='w-full h-14 focus:border-purple-500 focus:outline-none duration-200 rounded-md mb-4 bg-slate-900 text-slate-300 '
          placeholder='Search'
          type='search'
          name='search'
          id='search'
                  ...
                  games.map((game, index) => {
                    return (
                      <li
                        ...
                        className={'flex flex-row items-center px-6 py-2 border-slate-700 w-full duration-200 hover:bg-slate-700 hover:cursor-pointer hover:text-slate-300' + (index === games.length - 1 ? ' rounded-b-lg' : (index === 0 ? ' rounded-t-lg border-b' : ' border-b'))}
                      >
                        <img
                          alt={game.name}
                          className='w-10 h-auto rounded-md object-contain group-hover:scale-110 transition duration-300 ease-in-out'
                          src={game.box_art_url.replace('{width}', '20').replace('{height}', '30')}
                        />
